...
<?= $this->section('title') ?>Antrian Produksi<?= $this->endSection() ?>
<?= $this->section('content') ?>

<h5 class="fw-bold mb-4">Antrian Produksi</h5>

<?php if (empty($orders)): ?>
    <div class="text-center text-muted py-5">Tidak ada order dalam antrian produksi.</div>
<?php else: ?>
                ...
                <tr>
                    ...
                    <td>
                        <form action="<?= base_url('production/update-status/'.$o['id']) ?>" method="POST" class="d-inline">
                            <?= csrf_field() ?>
                            <input type="hidden" name="status_order" value="qc">
                            <button class="btn btn-sm btn-success">→ QC</button>
                        </form>
