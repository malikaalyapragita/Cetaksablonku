...
<?= $this->section('title') ?>Packing<?= $this->endSection() ?>
<?= $this->section('content') ?>

<h5 class="fw-bold mb-4">Packing</h5>

<?php if (empty($orders)): ?>
    <div class="text-center text-muted py-5">Tidak ada order dalam tahap packing.</div>
<?php else: ?>
                ...
                <tr>
                    ...
                    <td>
                        <form action="<?= base_url('production/update-status/'.$o['id']) ?>" method="POST" class="d-inline">
                            ...
                            <button class="btn btn-sm btn-success">→ Siap Kirim</button>
                        </form>
